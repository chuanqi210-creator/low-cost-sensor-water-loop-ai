...
OUT_DIR = PROJECT_ROOT / "outputs" / "agent20_campaign_telemetry"
...
def main() -> None:
    ...
    payload = {
        ...
        "campaign_telemetry": {
            "agent_name": telemetry_report.agent_name,
            "confidence": telemetry_report.confidence,
            "summary": telemetry_report.summary,
            "recommendations": telemetry_report.recommendations,
            "metrics": telemetry_report.metrics,
            "issues": [
                {
                    "sensor": issue.sensor,
                ...
                for issue in telemetry_report.issues
            ],
        },
    ...
    (OUT_DIR / "agent20_report.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

    state = control_report.metrics["current_control_state"]
    lines = [
        "# Agent 20 Campaign 遥测桥接模拟报告",
        "",
        f"- telemetry_summary: {telemetry_report.summary}",
    ...
    for update in telemetry_report.metrics["rolling_campaign_updates"]:
        lines.extend(
            [
                f"### update {update['campaign_id']}",
                "",
                f"- cut_point_batch_count: `{update['cut_point_batch_count']}`",
                f"- acceptance_passed: `{update['acceptance_passed']}`",
                f"- success_rate: `{update['success_rate']}`",
                f"- validation_staff_usage: `{update['validation_staff_usage']}`",
                f"- time_budget_usage: `{update['time_budget_usage']}`",
                f"- catalyst_spares_remaining: `{update['catalyst_spares_remaining']}`",
                f"- oxidant_stock_units_remaining: `{update['oxidant_stock_units_remaining']}`",
                f"- intake_pressure_multiplier: `{update['intake_pressure_multiplier']}`",
    ...
    for decision in control_report.metrics["rolling_decisions"]:
        lines.extend(
            [
                f"### campaign update {decision['campaign_id']}",
                "",
    ...
    for rec in control_report.recommendations:
        lines.append(f"- {rec}")
    (OUT_DIR / "agent20_report.md").write_text("\n".join(lines), encoding="utf-8")

    print(telemetry_report.summary)
    print(control_report.summary)
    for rec in control_report.recommendations:
        print(f"- {rec}")
    print(f"wrote {OUT_DIR / 'agent20_report.md'}")
